...
export class AndroidPluginBuildService implements IAndroidPluginBuildService {
	...
	// private static DEFAULT_PLUGIN_NAME = "myplugin";

	private getAndroidSourceDirectories(source: string): Array<string> {

		const getLsStats = this.$fs.getLsStats;
		///// Helpe methods to operate with android sourceset files (res, assets, java, etc.)
		function isAndroidSourceDirectory(src: string) {
			const sourceNameParts = src.split(path.sep);
			const dirName = sourceNameParts[sourceNameParts.length - 1];
			return (
				getLsStats(src).isDirectory() &&
				(dirName === "res" ||
					dirName === "java" ||
					dirName === "assets" ||
					dirName === "jniLibs")
	...
	private getDirectories(source: string): any {
	...
	public async buildAar(options: IBuildOptions): Promise<boolean> {
		...
		if (shouldBuildAar) {
			this.createDirIfDoesntExist(newPluginDir);
			this.createDirIfDoesntExist(path.join(newPluginDir, "src"));
			this.createDirIfDoesntExist(newPluginMainSrcDir);
			...
			try {
				await options.platformData.platformProjectService.executeCommand(options.platformData.projectRoot, localArgs);
